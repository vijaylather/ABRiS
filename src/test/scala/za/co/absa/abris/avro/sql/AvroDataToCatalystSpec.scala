...
class AvroDataToCatalystSpec extends AnyFlatSpec with Matchers with BeforeAndAfterEach {
  ...
  it should "not print schema registry configs in the spark plan" in {
    ...
    AbrisSparkColumnHelper.columnToExpr(column).toString() should not include sensitiveData
  }
  ...
  it should "use the default schema converter by default" in {
    ...
    val column = from_avro(col("avroBytes"), fromAvroConfig)
    AbrisSparkColumnHelper.columnToExpr(column).dataType shouldBe expectedDataType
  }
  ...
  it should "use a custom schema converter identified by the short name" in {
    ...
    val column = from_avro(col("avroBytes"), fromAvroConfig)
    AbrisSparkColumnHelper.columnToExpr(column).dataType shouldBe DummySchemaConverter.dataType
  }
  ...
  it should "throw an error if the specified custom schema converter does not exist" in {
    ...
    val ex = intercept[ClassNotFoundException](AbrisSparkColumnHelper.columnToExpr(from_avro(col("avroBytes"), fromAvroConfig)).dataType)
  ...
  it should "be serializable" in {
    ...
    val avroDataToCatalyst = AbrisSparkColumnHelper.columnToExpr(from_avro(col("col"), config))
